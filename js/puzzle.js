...
const images = [
    '../juego/imagenes/da-vinci-retrato.jpg',
    '../juego/imagenes/frida-kahlo.jpg',
    '../juego/imagenes/van-gogh.jpg'
];
...
function updateScoreDisplay() {
    document.querySelector("#score").innerText = "Puntaje: " + score;
}
...
function actualizarPuntaje(valor) {
    ...
    // Si se terminan los puntos, termina el juego
    if (score === 0) {
        terminarJuego();
    }
}
...
function terminarJuego() {
    ...
    setTimeout(() => {
        alert("¡Se acabaron los puntos! La ronda termina sin ganador.");
...
function moverPieza(indice) {
    ...
    if (distancia === 1) {
        ...
        // descuenta puntos por movimiento
        actualizarPuntaje(-5);
        comprobarVictoria();
    }
}
...
function comprobarVictoria() {
    ...
    // si se llega hasta acá,, el puzzle se ha resuelto
    setTimeout(() => {
        container.classList.add("fade");
        setTimeout(() => {
...
// Función para mostrar una sugerencia (resalta la pieza adyacente al hueco)
...
// Asigna eventos a los botones de pista
document.querySelector("#hintBtn").addEventListener("click", mostrarPista);
document.querySelector("#sugerenciaBtn").addEventListener("click", mostrarSugerencia);

// Inicializa el puzzle y el puntaje al cargar la página
